...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="tes"; }
echo 'tes() { "$HOME/core-token/packages/core/bin/run" "$@" --token="tes"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/Highjhacker/a_bc_de_f_g_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/Highjhacker/a_bc_de_f_g_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@tes"
rm -rf "$HOME/.config/@aBcDeFG"
rm -rf "$HOME/.config/tes-core"
